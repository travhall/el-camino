...
describe('PDPUIManager', () => {
  ...
  describe('updateInventoryDisplay', () => {
    it('hides remaining-count for gift cards', () => {
      ...
      expect(
        document.getElementById('remaining-count')?.classList.contains('hidden')
      ).toBe(true);
    });
    ...
    it('hides the in-cart count when inCart is 0', () => {
      ...
      expect(
        document.getElementById('cart-quantity')?.classList.contains('hidden')
      ).toBe(true);
    });
    ...
    it('injects the out-of-stock message only for OUT_OF_STOCK', () => {
      ...
      ).toContain('currently out of stock');
    });
  ...
  describe('updatePriceDisplay', () => {
    it('sets the formatted price and hides original-price-display when no saleInfo', () => {
      ...
      expect(document.getElementById('price-display')?.textContent).toContain(
        '19.99'
      );
      expect(
        document
          .getElementById('original-price-display')
  ...
  describe('updateGalleryThumbnails', () => {
    it('hides the container for 0 images', () => {
      ...
      expect(
        document
          .getElementById('gallery-thumbnails')
          ?.classList.contains('hidden')
      ).toBe(true);
    });
