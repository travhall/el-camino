...
import type { ProductPageData } from '@/lib/product/pdpEvents';
...
export class QuickViewController {
  ...
  private productData: ProductPageData | null = null;
  ...
  private initializeProduct(
    ...
    // variations (e.g. a deck in multiple sizes) — assign them to variation.images. cSpell:ignore colour
  ...
  private renderVariations(product: Product): void {
    ...
    if (product.variations.length > 1) {
      attributeTypes.forEach((attributeType) => {
        ...
        heading.className = 'text-sm font-medium text-(--content-heading) mb-2';
  ...
  private updateProductUI(): void {
    if (!this.currentVariation || !this.currentProduct || !this.productData)
      return;
  ...
  private buildProductData(variation: ProductVariation) {
      ...
      button.onclick = () => {
        ...
        const variantSlug = variationName
          ? variationName
              ...
              .replace(/^-|-$/g, '')
          : null;
        const url = variantSlug ? `${baseUrl}?variant=${variantSlug}` : baseUrl;
  ...
  async addToCart(): Promise<void> {
    ...
    try {
      ...
      if (result.success) {
        showNotification(result.message || 'Added to cart', 'success', 3000, {
          label: 'View Cart',
          href: '/cart',
        });
  ...
  setupEventHandlers(): void {
    ...
    const thumbnailContainer = document.getElementById('quick-view-thumbnails');
