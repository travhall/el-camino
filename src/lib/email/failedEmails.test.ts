...
const makeRecord = (
  overrides?: Partial<FailedEmailRecord>
): FailedEmailRecord => ({
  orderId: 'order-123',
  order: { id: 'order-123', lineItems: [] } as unknown as Order,
  contact: {
...
describe('failedEmails', () => {
  ...
  describe('storeFailedEmail', () => {
    ...
    it('coerces non-Error throws to strings', async () => {
      ...
      const order = { id: 'order-xyz' } as unknown as Order;
      const contact = {
        email: 'a@b.com',
        name: 'A',
  ...
  describe('getFailedEmail', () => {
    it('returns the record when it exists', async () => {
      ...
      expect(mockBlobStore.get).toHaveBeenCalledWith('order-123', {
        type: 'json',
      });
    });
