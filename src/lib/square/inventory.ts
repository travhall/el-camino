...
// These functions maintain existing API while using the new service layer
// ============================================================================

import { inventoryService } from "./services";

/**
 * @deprecated Use inventoryService.checkInventory() instead
 */
export async function checkItemInventory_legacy(variationId: string): Promise<number> {
  console.warn(
    "checkItemInventory is deprecated. Use inventoryService.checkInventory()"
  );
  return inventoryService.checkInventory(variationId);
}

/**
 * @deprecated Use inventoryService.isInStock() instead
 */
export async function isItemInStock_legacy(variationId: string): Promise<boolean> {
  console.warn(
    "isItemInStock is deprecated. Use inventoryService.isInStock()"
  );
  return inventoryService.isInStock(variationId);
}

/**
 * @deprecated Use inventoryService.checkBulkInventory() instead
 */
export async function checkBulkInventory_legacy(
  variationIds: string[]
): Promise<Record<string, number>> {
  console.warn(
    "checkBulkInventory is deprecated. Use inventoryService.checkBulkInventory()"
  );
  return inventoryService.checkBulkInventory(variationIds);
}

/**
 * @deprecated Use inventoryService.getProductStockStatus() instead
 */
export async function getProductStockStatus_legacy(product: Product): Promise<{
  isOutOfStock: boolean;
  hasLimitedOptions: boolean;
}> {
  console.warn(
    "getProductStockStatus is deprecated. Use inventoryService.getProductStockStatus()"
  );
  return inventoryService.getProductStockStatus(product);
}
