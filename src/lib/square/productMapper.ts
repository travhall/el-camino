...
export function mapCatalogItemsToProducts(
  ...
  return productsWithBasicInfo.map((p) => {
    ...
    const variations =
      (item?.type === 'ITEM' ? item.itemData?.variations : undefined)?.filter(
        isItemVariation
      ) ?? [];
    ...
    const productVariations = variations
      ...
      .map((v) => {
        ...
        const regularPrice = variationPrice
          ? Number(variationPrice.amount) / 100
          : 0;
        const saleInfo = extractSaleInfo(v.customAttributeValues, regularPrice);
        const rawVariationImageIds: string[] =
        ...
        return {
          ...
          images:
            variationImageUrls.length > 1 ? variationImageUrls : undefined,
