...
describe('logError', () => {
  ...
  it('logs the error type and message via console.error', () => {
    ...
    logError(
      createError(ErrorType.AUTH_ERROR, 'Not authorized', { source: 'auth' })
    );
  ...
  it('summarizes an Error originalError instead of dumping the raw object', () => {
    ...
    logError(
      createError(ErrorType.UNKNOWN, 'wrapped', { originalError: original })
    );
    const [, details] = spy.mock.calls[0];
    expect(details.originalError).toMatchObject({
      name: 'Error',
      message: 'root cause',
    });
  });
...
describe('processClientError', () => {
  it('classifies timeout errors', () => {
    const err = processClientError(
      new Error('ETIMEDOUT while fetching'),
      'fetchProduct'
    );
  ...
  it('classifies network errors', () => {
    const err = processClientError(
      new Error('ENOTFOUND api.square.com'),
      'fetchProduct'
    );
