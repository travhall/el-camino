...
// Minimal stand-in for the sale-attribute shape `mockExtractSaleInfo` reads —
// the real `customAttributeValues` shape (SquareCatalogCustomAttributes) is
// irrelevant here since extractSaleInfo itself is mocked out below.
type SaleAttrs = { __sale?: number };

const variation = (
  id: string,
  cents: number,
  attrs: SaleAttrs | undefined = undefined
) => ({
  id,
  type: 'ITEM_VARIATION',
...
beforeEach(() => {
  vi.clearAllMocks();
  mockExtractSaleInfo.mockImplementation((attrs: SaleAttrs | undefined) =>
