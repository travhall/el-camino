...
describe('filterSaleProducts', () => {
  const variation = (saleInfo?: SaleInfo): Partial<ProductVariation> => ({
    saleInfo,
  });
    ...
    const products = [
      {
        ...
        variations: [
          variation(undefined),
          variation({ salePrice: 10, originalPrice: 20, discountPercent: 50 }),
        ],
      },
  ...
  it('returns an empty array when no products have a sale variation', () => {
    const products = [
      { id: '1', variations: [variation(undefined)] },
    ] as Product[];
...
describe('extractFilterOptions', () => {
  ...
  it('ignores products with no brand', async () => {
    mockFetchCategoryHierarchy.mockResolvedValue([]);
    const { brands } = await extractFilterOptions([
      { brand: undefined } as Product,
    ]);
    expect(brands).toEqual([]);
  });

  it('degrades gracefully to brand-only options when category hierarchy fetch fails', async () => {
    mockFetchCategoryHierarchy.mockRejectedValue(
