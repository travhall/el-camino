...
const mockSocial = [
  { platform: 'instagram', url: 'https://x', icon: 'uil:instagram' },
];
...
vi.mock('@/lib/contactInfo', () => ({
  getContactInfo: (...a: unknown[]) => getContactInfo(...a),
}));
vi.mock('@/lib/socialLinks', () => ({
  getSocialLinks: (...a: unknown[]) => getSocialLinks(...a),
}));
vi.mock('@/lib/shopHours', () => ({
  getShopHours: (...a: unknown[]) => getShopHours(...a),
}));
vi.mock('@/lib/structuredData', () => ({
  getStructuredData: (...a: unknown[]) => getStructuredData(...a),
}));
vi.mock('@/lib/saleVisibility', () => ({
  getSalePageVisible: (...a: unknown[]) => getSalePageVisible(...a),
}));
vi.mock('@/lib/shopVisibility', () => ({
  getShopPageVisible: (...a: unknown[]) => getShopPageVisible(...a),
}));
...
describe('getSiteContext', () => {
  ...
  it('calls each underlying getter exactly once per request and passes resolved values into getStructuredData', async () => {
    ...
    expect(getStructuredData).toHaveBeenCalledWith(
      mockContact,
      mockSocial,
      mockHours
    );
