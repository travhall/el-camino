...
export class BudgetManager {
  ...
  public checkCacheEffectiveness(): void {
    ...
    try {
      const metrics: CacheMetricEntry[] = JSON.parse(localStorage.getItem('cacheMetrics') || '[]');
      const recent = metrics.filter((m: CacheMetricEntry) => Date.now() - m.timestamp < 300000); // Last 5 minutes
      
      if (recent.length > 10) {
        const hitRate = recent.filter((m: CacheMetricEntry) => m.hit).length / recent.length;
        const avgResponseTime = recent.reduce((sum: number, m: CacheMetricEntry) => sum + m.responseTime, 0) / recent.length;
        ...
        console.log(`[CacheMonitor] Hit Rate: ${(hitRate * 100).toFixed(1)}% | Avg Response: ${avgResponseTime.toFixed(0)}ms | Samples: ${recent.length}`);
        
        // Alert on concerning trends
        if (hitRate < 0.3) {
          console.warn(`[CacheAlert] Low cache hit rate: ${(hitRate * 100).toFixed(1)}%`);
        }
        
        if (avgResponseTime > 1000) {
          console.warn(`[CacheAlert] Slow API responses: ${avgResponseTime.toFixed(0)}ms average`);
        }
      }
    } catch (error) {
      console.warn('[CacheMonitor] Failed to check cache effectiveness:', error);
    }
  }
}
