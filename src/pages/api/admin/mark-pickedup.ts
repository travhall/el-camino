...
export const POST: APIRoute = async ({ request, cookies, redirect }) => {
  ...
  try {
    ...
    if (!fulfillment?.uid)
      throw new Error('No active PICKUP fulfillment found');
  ...
  const currentIdx = PICKUP_STATES.indexOf(
    currentState as (typeof PICKUP_STATES)[number]
  );
  const targetIdx = PICKUP_STATES.indexOf('COMPLETED');
  const steps = PICKUP_STATES.slice(Math.max(currentIdx + 1, 0), targetIdx + 1);
  ...
  try {
    ...
    const detail =
      ...
      'unknown';
    console.error(
      `[mark-pickedup] Square update failed for ${orderId}:`,
      JSON.stringify(err, null, 2)
    );
    return redirect(
      `/admin/orders/pickups?error=update&detail=${encodeURIComponent(detail)}`
    );
  }
