...
beforeEach(() => {
  ...
  vi.mocked(getSocialLinks).mockResolvedValue(existingLinks as any);
...
describe("POST /api/admin/social", () => {
  it("redirects to login when not authenticated", async () => {
    ...
    const res = await POST(makeContext({ action: "add", platform: "twitter", url: "https://twitter.com/shop" }));
  ...
  describe("action=add", () => {
    it("redirects with error=missing-fields when platform or url is absent", async () => {
      const res = await POST(makeContext({ action: "add", platform: "twitter" }));
      expect(res.status).toBe(302);
    ...
    it("redirects with error=invalid-url when the url has a disallowed scheme", async () => {
      const res = await POST(makeContext({ action: "add", platform: "twitter", url: "javascript:alert(1)" }));
      expect(res.status).toBe(302);
    ...
    it("redirects with error=duplicate when platform already exists", async () => {
      const res = await POST(makeContext({ action: "add", platform: "instagram", url: "https://instagram.com/other" }));
      expect(res.status).toBe(302);
    ...
    it("saves the new link and redirects with saved=1 on success", async () => {
      ...
      expect(saveSocialLinks).toHaveBeenCalledWith([
        ...existingLinks,
        expect.objectContaining({ platform: "twitter", url: "https://twitter.com/shop" }),
      ]);
      expect(res.status).toBe(302);
  ...
  describe("action=update-url", () => {
    it("redirects with error=invalid-url for a bad scheme", async () => {
      const res = await POST(makeContext({ action: "update-url", platform: "instagram", url: "ftp://bad.com" }));
      expect(res.status).toBe(302);
      expect(res.headers.get("Location")).toContain("error=invalid-url");
    });

    it("updates the url for the matching platform and redirects with saved=1", async () => {
      const res = await POST(makeContext({ action: "update-url", platform: "instagram", url: "https://instagram.com/new" }));
      expect(saveSocialLinks).toHaveBeenCalledWith([
        expect.objectContaining({ platform: "instagram", url: "https://instagram.com/new" }),
      ]);
      expect(res.status).toBe(302);
      expect(res.headers.get("Location")).toContain("saved=1");
    });
