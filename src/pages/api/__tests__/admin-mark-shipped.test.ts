...
const { mockIsAdmin, mockOrdersGet, mockOrdersUpdate, mockSendShipping } = vi.hoisted(() => ({
  mockIsAdmin: vi.fn(),
  mockOrdersGet: vi.fn(),
...
describe("POST /api/admin/mark-shipped", () => {
  ...
  it("walks state machine to COMPLETED and sends shipping confirmation", async () => {
    ...
    const res = await POST(makeContext({ orderId: "order-1", trackingNumber: "1Z999", carrier: "UPS" }));
