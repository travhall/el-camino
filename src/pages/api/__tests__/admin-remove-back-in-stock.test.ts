...
const REMOVED = [
  { email: "a@example.com", productId: "prod-1", productTitle: "Baker Deck", productUrl: "/products/baker" },
];
...
describe("POST /api/admin/remove-back-in-stock", () => {
  beforeEach(() => {
    ...
    (removeAllSubscriptionsForProduct as ReturnType<typeof vi.fn>).mockResolvedValue([]);
  });
  ...
  it("returns 401 JSON when unauthenticated and accept is application/json", async () => {
    mockIsAdmin.mockReturnValue(false);
    const res = await POST(makeContext({ formData: { productId: "prod-1" }, acceptJson: true }));
  ...
  it("removes all subscribers and redirects with removed count (form request)", async () => {
    (removeAllSubscriptionsForProduct as ReturnType<typeof vi.fn>).mockResolvedValue(REMOVED);
    const res = await POST(makeContext({ formData: { productId: "prod-1" } }));
