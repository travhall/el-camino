...
vi.mock("@/lib/shopStatus", () => ({
  getShopStatusConfig: vi.fn().mockResolvedValue({ mode: "auto", until: undefined, holidays: [] }),
...
describe("POST /api/admin/shop-status (admin)", () => {
  ...
  describe("action=save-override", () => {
    ...
    it("returns 400 for invalid mode", async () => {
      const res = await POST(makeContext({ action: "save-override", mode: "maybe" }));
      expect(res.status).toBe(400);
    });
  ...
  describe("action=add-holiday", () => {
    it("adds a one-time holiday and redirects with saved=holiday", async () => {
      const res = await POST(
        makeContext({ action: "add-holiday", label: "Christmas", type: "one-time", date: "2026-12-25" }),
      );
      expect(saveShopStatusConfig).toHaveBeenCalledWith(
        expect.objectContaining({
          holidays: expect.arrayContaining([expect.objectContaining({ label: "Christmas" })]),
        }),
      );
    ...
    it("redirects with error=missing-label when label is absent", async () => {
      const res = await POST(
        makeContext({ action: "add-holiday", type: "one-time", date: "2026-12-25" }),
      );
    ...
    it("redirects with error=invalid-date for invalid one-time date", async () => {
      const res = await POST(
        makeContext({ action: "add-holiday", label: "Bad", type: "one-time", date: "not-a-date" }),
      );
    ...
    it("adds recurring holiday with MM-DD format", async () => {
      const res = await POST(
        makeContext({ action: "add-holiday", label: "New Years", type: "recurring", recurring: "01-01" }),
      );
      expect(saveShopStatusConfig).toHaveBeenCalledWith(
        expect.objectContaining({
          holidays: expect.arrayContaining([expect.objectContaining({ recurring: "01-01" })]),
        }),
      );
      expect(res.headers.get("Location")).toContain("saved=holiday");
    });
  ...
  describe("action=remove-holiday", () => {
    it("removes holiday by id and redirects with removed=holiday", async () => {
      ...
      const res = await POST(makeContext({ action: "remove-holiday", id: "hol-1" }));
      expect(saveShopStatusConfig).toHaveBeenCalledWith(
